...
The IOxOS IFC1211 is a VME64x compliant board that is currently being used to develop EPICS IOCs because its hardware mimics that of the microTCA.4 AMC card currently under development.
...
The PON FPGA is preconfigured and is supposed to manage the programming procedure of the other two FPGAs and the reset signal for the T2081.
...
\begin{table}[!hb]
  ...
  \begin{tabular}{l|l|l}
    ...
    IFC1211                   & \texttt{ICS TAG-382} or \texttt{ICS TAG-382}                       & 16370107 or 16370109       \\\midrule
    VME Crate                 & Hostname : icsb-vmeifc1210-529 or icsb-vmeifc1210-528              &                            \\\midrule
...
Figure~\ref{fig:ser_cable} shows the usual cable enumeration and Table~\ref{table:ser_cable_tab} shows the required connections at both ends assuming that the cable end at the Moxa NPort device follows the coloring scheme previously shown.
...
\begin{table}[!hb]
  ...
  \begin{tabular}{|c|c|c|c|c|}\cline{2-5}
  	\multicolumn{1}{c|}{}
  	& \multicolumn{2}{c|}{\cellcolor{gray!20}\bfseries\sffamily Moxa NPort 5610 (ics-moxabox-01)} 
  	& \multicolumn{2}{c|}{\cellcolor{gray!20}\bfseries\sffamily IFC1211} \\
  	\hline
  	\cellcolor{gray!20}{\bfseries\sffamily Pin n$^{\circ}$}
  	& Color
  	& Description
  	& Color
  	& Description \\
  	...
  	\hline\rowcolor{LightCyan}
  	\multirow{2}{*}{6} & \multirow{2}{*}{green solid} & \multirow{2}{*}{DCD (in)} & none (do not connect & \multirow{2}{*}{GNDC} \\
  	\rowcolor{LightCyan}
...
The white/orange stripe cable is not routed to any pin on the IFC1211 side and is actually left dangling, in addition one of the GND pins is not connected but so far no problem was detected related to this.


\clearpage
\section{Software}
Table~\ref{table:swlist} shows the Software list and its environment. It is not necessary to have the same Linux OS or the same kernel version to be able to boot the IFC1211 board, but there might be a few differences in terms of the configuration and installation procedure of the different services.
\begin{table}[!htb]
  \centering
  \begin{tabular}{l|l}
    \toprule
...
\iffalse
    ...
    TFTP server               & version : \texttt{1} / srcversion                               \\\midrule
    NFS server                & \texttt{1.8.0}                                                  \\\midrule
\fi
...
The file uImage\_2080 is a Linux kernel image binary file with the U-boot boot-loader wrapper. The t2080ifc.dtb is a device tree binary file that provides informations about the available devices, their access address and the compatible drivers.
...
\iffalse
This chapter provides the minimal information necessary to configure the TFTP and NFS servers, modify the U-boot environment to allow it to load a script and create such a compiled script file. 
\fi
This chapter provides the informations necessary to boot the IFC1211 board. The modifications required on the U-boot environment to automatically load a script will be described and the steps necessary to compile such a script file will be covered.
All the procedures described require to have access to the \texttt{ics-boot-01.esss.lu.se} machine, available in the ics-lab network.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.80\textwidth]{./pictures/ser_ifc_con.eps}
	\caption{
		Serial interface location on the IFC1211.
	}
...
To do so open a web browser and access the \texttt{http://10.4.3.90/}, under "Serial Settings" choose the appropriate port number and modify the values to reflect the IFC1211 serial settings previosly described.
Connect the appropriate end of the Ethernet cable to the IFC1211 and to the configured port on the MoxaBox.
The driver required to interface to the MoxaBox are already installed in the \texttt{ics-workstation (10.4.3.18)}, if you need to setup the communication to this device on your pc the relevant informations are described in the ESS wiki in the \cite{ASDLL} document.
...
This default environment does not allow the load of a boot script at startup, in order to do so we need to modify it. The following sections will describe how to do that but first let focus on the script itself.
...
The script can contain any command that could also be issued in the boot loader shell, to view the list available commands enter \texttt{?} in the shell.
...
The \texttt{ics-boot-01.esss.lu.se} machine provides all these services, its export directory is \texttt{/export} and its organization is as follows
\begin{lstlisting}[style=termstyle]
...
To keep the files organized copy the uImage\_2080 and t2080ifc.dtb into the \texttt{images/ifc1211-default} folder and untar the root file system into the \texttt{nfsroots/ifc1211}.
...
\begin{lstlisting}[style=termstyle]
[srvr-machine@localhost ~]$ touch uboot_ifc1211.script
[srvr-machine@localhost ~]$ vi uboot_ifc1211.script
...
If the mkimage executable is not available in the location shown above a way to find it is by running the following command.

\begin{lstlisting}[style=termstyle]
[username@ics-boot-01 ~]$ find / ! -readable -prune -o -path *u-boot* -name mkimage -print
\end{lstlisting}

This will display all available files named \texttt{mkimage} that are located in a folder with \texttt{u-boot} in his path.
...
The generated script is now ready to be retrieved and loaded from the server by the bootloader but it has to be instructed to do so and to accomplish this the default Environment has to be modified.
Connect to the IFC1211 board serial interface as described previously and type the following two commands:
...
The \texttt{env-\$eth1addr.scr} is introduced to reflect what the default naming scheme will look like when deploying the boards in the facility and translates to a file name that includes the MAC address of the interface \texttt{eth1}, in this way any script file will be associated only to a single board.

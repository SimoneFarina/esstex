...
\begin{table}[!htb]
  ...
  \end{tabular}
  \caption[]{The EVR FPGA Firmware Version Register in Reference \citep[see][p66]{MRFEVENTSYSTEMDC}}
  \label{table:fwinfo}
\end{table}
...
In this section, one should consider in order to start the EPICS IOC for the MRF PCIe-EVR-300 under EEE: 1) the EPICS database file, and 2) the EPICS start-up script. All files are located in a directory, where an user can create, e.g., in the ICS Lab, 
...
+   field( VAL, "$(Link-Clk-SP=88.0532(*@\label{newlinkclockfreq}@*), undefined)")}
    field(EGU , "MHz")
    field(LINR, "LINEAR")
    field(ESLO, "1e-6")
    field(HOPR, "150")
...
Listing~\ref{list:st.cmd} shows the start-up script of the EPICS IOC, the MRF PCIe-EVR-300 Identification Numbers, shown in Table~\ref{table:pciidnumber}, are used in the script. And the script is located in the directory in Listing~\ref{list:pwd}. 


\begin{lstlisting}[
    style=termstylenumber,
    label={list:st.cmd},
    caption={Start-up script \texttt{evr300\_0.cmd}. Line \ref{pciid0}-\ref{pciid3} should be matched to Table~\ref{table:pciidnumber}. The EPICS Database file path should be checked in Line \ref{dbpwd}.}
  ]
...
\begin{lstlisting}[style=termstyle]
  ...
  ics-essiip-01>
\end{lstlisting}
One could check the entire environment setting in the output also. In addition, the PCI information available within this EPICS IOC through \texttt{devPCIShow} command as follows:
\begin{lstlisting}

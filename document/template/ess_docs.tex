...
\begin{document}
%\newcommand{\technumber}{
%  \st{RAON Control-Document Series}\\
%  \st{Revision : v1.1,   Release : June 18, 2014}}
%  }

\title{\textbf{MRF PCIe-EVR-300}}

\author{Jeong Han Lee\thanks{citadel.lee@gmail.com, I don't work for this project, but it should be good for FAI user community. }\\
  \st{Rare Isotope Science Project}\\
  \st{Institute for Basic Science, Daejeon, South Korea}
}
\date{\today}
...
\chapter{TEST}
\section{Hardware setup and specification}
Tests made with:
\begin{itemize}
\item MRF PCIe-EVR-300
	...
	\end{itemize}
\item Installed in Kontron iPC in ESSIIP Control Rack in ICS lab (ics-essiip-01).
\item Connected to MRF fan-out; master is cPCI EVG
\item Using output module IFB-300
	\begin{itemize}
...
-+-[0000:05]---00.0
 \-[0000:00]-
\end{verbatim}
The PCI information available from the IOC is:
\begin{verbatim}
...
\item FW version 0x17000008
\item evr-pcie-300.db (/opt/epics/modules/mrfioc2/3.0.0/db/evr-pcie-300.db)
\end{itemize}


\section{Startup configuration}
The cPCI EVG is set to send event 14 at 14 Hz and event clock frequency of 88.0525 MHz.\\
Since this frequency is different to the default in \verb|evr-pcie-300.db|, the file is copied to the current folder and the link clock frequency is set to 88.0525 MHz (line 111):\\
\verb|field( VAL , "$(Link-Clk-SP=88.0525,undefined)")|\\

MRF kernel module loaded with:\\\verb|$modprobe mrf|\\
...
mrmEvrSetupPCI($(EVR), $(EVR_PCIDOMAIN), $(EVR_PCIBUS), $(EVR_PCIDEVICE),
        $(EVR_PCIFUNCTION))
...
dlload         /opt/epics/modules/environment/1.8.0/3.14.12.5/lib/
        centos7-x86_64/libenvironment.so
dbLoadDatabase /opt/epics/modules/environment/1.8.0/3.14.12.5/dbd/
        environment.dbd
...
require: Loading library /opt/epics/modules/devlib2/2.6.0/3.14.12.5/lib/
        centos7-x86_64/libdevlib2.so.
...
require: Loading library /opt/epics/modules/mrfioc2/3.0.0/3.14.12.5/lib/
        centos7-x86_64/libmrfioc2.so.
...
\section{Test 2 - monitoring event reception on the EVR}
...
\begin{verbatim}
dbLoadRecords("evr-pulserMap.template", "EVR=$(EVR), SYS=$(SYS),
        PID=0, F=Trig, ID=0, EVT=14")
dbLoadRecords("evr-pulserMap.template", "EVR=$(EVR), SYS=$(SYS),
        PID=1, F=Trig, ID=0, EVT=14")
\end{verbatim}
...
\begin{figure}[!ht]
  ...
  \caption{14 Hz signal with delay}
  \label{fig:delay}
\end{figure}
...
%Run {\tt make attach} to embed the article's source code into its PDF file.
%They can be extracted either with Acrobat Reader or with {\tt pdftk article.pdf unpack\_files output /tmp}.

...
\item The purpose of this document is to describe the engineering procedure for how the MRF PCIe-EVR-300 board will be integrated in cooperation with  ESS EPICS Environment (EEE).
...
This document is targeted to ICS engineers and technical stakeholders. It is assumed that the target audience has a technical background in a MRF Timing System and in EPICS development.
...
Before describing the engineering procedure in order to integrate the MRF PCIe-EVR-300 board, it is mandatory to have proper system environment that consists of specific hardware and software lists. Here we will show the hardware and software lists, their block diagrams, and their setup in the ICS lab at ESS.

\clearpage

\section{Hardware List}
Table~\ref{table:hwlist} shows the hardware information. \texttt{TAG} is the prefix of the ICS internal inventory system. 
\begin{table}[!htb]
  \centering
  \begin{tabular}{l|l|l}
    \toprule
    ...
    Kontron Industrial PC     & ics-essiip-01          & \\\midrule
    ...
    Optical cables            &               & \\\midrule
    LEMO cables               &               & \\\midrule
    LEMO to BNC Adapters      &               & \\\midrule
    Oscilloscope              &               & \\\bottomrule
  \end{tabular}
  \caption[]{Hardware List. One can use only EVG without FOUT. And the form factor and version of EVG can be changeable. PC can also changeable, but the PCIe slot of a PC should support more than 10W.}
  \label{table:hwlist}
\end{table}
...
Table~\ref{table:swlist} shows the Software and Firmware information.
...
\begin{table}[!htb]
  ...
  \begin{tabular}{l|l}
    ...
    Kernel                & 3.10.0-229.7.2.rt56.141.6.el7.centos.x86\_64        \\\midrule
    EEE \& EPICS Base     & \texttt{1.8.0}  \& \texttt{3.14.12.5}               \\\midrule
    mrfioc2               & EEE module ver. 3.0.0                               \\\midrule
    EVR FPGA Firmware Version Register & \texttt{0x17000008}                    \\\midrule
    ...
    EVR Firmware ID - 300 &  \texttt{0x1700}\underline{\textbf{00}}\texttt{08}  \\\midrule
    EVR Revision ID       &  \texttt{0x170000}\underline{\textbf{08}}           \\\bottomrule
  \end{tabular}
  \caption[]{Software List. Please see the detailed information of the EVR FPGA Firmware Version Register in Reference \citep[][p66]{MRFEVENTSYSTEMDC}}
  \label{table:swlist}
\end{table}
...
\section{PCIe EVR-300 Board Installation}
...
\chapter{System Deployment Checklist}
****************************************\\
...
\begin{table}[!htb]
  ...
  \begin{tabular}{c|p{0.8\linewidth}}
    \toprule
    No. & Steps  \\    \toprule
    0   & checking the EVR-EVG link and setting event clock and link frequency \\\midrule
    0   &  monitoring event reception on the EVR \\\midrule
    0   &  reacting to events by producing digital triggers \\\bottomrule
  \end{tabular}
  \caption[]{System Deployment Checklist}
  \label{table:checklist}
\end{table}


\section{Step 1}
checking the EVR-EVG link and setting event clock and link frequency
\begin{lstlisting}
...
\section{Step 2}
- monitoring event reception on the EVR
...
\section{Step 3}
reacting to events by producing digital triggers
...
\end{lstlisting}
The solution can be found in the "Timing System: User Manual - Troubleshooting" wiki page \cite{TSESSWIKI}.

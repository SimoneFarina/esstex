...
\chapter{Overview}
ICS at ESS uses the Micro Research Finland (MRF) Timing System{\footnote{\url{http://www.mrf.fi/}}} as its timing system of the ESS site. The consistent and up-to-date engineering manual is essential for the ESS Timing system. 
...
\begin{itemize}
  ...
  \item This document provides the specification of an EVR, and its configuration in cooperation with ESS EPICS Environment (EEE).
  \item This document attempts to maintain consistency with existing ESS Timing system hardware as far as possible. 
  \item This document might be a part of the documentation system and maps to ICS Breakdown structure (WBS) in future.

\end{itemize}


\chapter{System Description}
MRF Technical Refenrece \citep[see][p45]{MRFEVENTSYSTEMDC} explained Event Recievers and wrote :
...
\item VME-EVR-300
\end{itemize}
And this document covers only PCIe-EVR-300 or PCIe-EVR-300DC\footnote{Delay Compenstation (DC) functionality exists in its firmware, so we mix their name in this document in general.}.
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.90\textwidth]{./pictures/pcie_evr_300dc.eps}
  \caption{
...
The Hardware information is  summarized in Table~\ref{table:hwlist}. 
...
\begin{table}[!htb]
  ...
  \begin{tabular}{l|l|l}
    ...
    \toprule
    MRF PCIe-EVR-300      & ICS TAG-146   & K114029\\\midrule
    Kontron Industrial PC & ics-essiip-01 & \\\midrule
    MRF IFB-300           & ICS TAG-336   & K472056  \\\midrule
    MRF cPCI-EVG-230      & ICS TAG-26    & E283016  \\\midrule
    MRF cPCI-FOUT-12      & ICS TAG-27    & H385013 \\\midrule
    Optical cables        &               & \\\midrule
    LEMO cable            &               & \\\midrule
    LEMO to BNC adaptor   &               & \\\midrule
  \end{tabular}
...
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.9\textwidth]{./pictures/hw-setup.eps}
  \caption{
    Hardware Setup.
  }
  \label{fig:hw-setup}   
...
The Software and Firmware information are summarized in Table~\ref{table:swlist}. 
\begin{table}[!hb]
  \centering
  \begin{tabular}{l|l}
    \toprule
    Device & Info \\    \toprule
    CentOS Linux          & 7.1.1503                                     \\\midrule
    Kernel                & 3.10.0-229.7.2.rt56.141.6.el7.centos.x86\_64  \\\midrule
    EEE \& EPICS Base     & 1.8.0  \& 3.14.12.5                           \\\midrule
    mrfioc2               & 3.0.0                                         \\\midrule
    EVR FPGA Firmware Version Register & 0x17000008 \\\midrule
    EVR                   &  0x\textbf{1}7000008  \\\midrule
    Form Factor           &  0x1\textbf{7}000008  \\\midrule
    EVR Firmware ID       &  0x1700\textbf{00}08  \\\midrule
    EVR Revision ID       &  0x170000\textbf{08} \\\midrule
  \end{tabular}
  \caption[]{Software List. Please see the detail information of EVR FPGA Firmware Version Register in Reference \citep[][p66]{MRFEVENTSYSTEMDC}}
  \label{table:swlist}
\end{table}
...
\section{Test environment description}



\section{PCI information}
...
\section{Test 1 - checking the EVR-EVG link and setting event clock and link frequency}
\begin{lstlisting}
...
\section{Test 3 - reacting to events by producing digital triggers}

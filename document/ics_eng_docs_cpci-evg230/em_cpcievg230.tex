...
\section{Scope}
\begin{itemize}
\item This document identifies one of the MRF Timing Event Generators (EVG) that needs to be configured for an ESS subsystem that needs synchronous frequencies, trigger signals and sequences of events \cite{MRFEVENTSYSTEMDC} \textcolor{magenta}{this ref needs to be changed in multiple places along the document}.
...
MRF Technical Reference \citep[see][p9]{MRFEVENTSYSTEMDC} explained Event Generators and wrote :
...
\textcolor{magenta}{this text is copied from ref:MRFEVENTSYSTEMDC, which is not applicable here (DC); afaik only the "events received from an upstream Event Generator" source of events is not applicable}

ICS uses and will use the following different types of EVG \textcolor{magenta}{update this list, maybe ask Han?}:
\begin{itemize}
...
And the scope of this document is to cover the cPCI-EVG-230 board.


\section{cPCI-EVG-230}
Figure~\ref{fig:pcie-evr300dc} \textcolor{magenta}{update this figure and dimmensions} shows the rough physical dimensions $104\times 79~\mathrm{mm}{}^2$ of the cPCI-EVG-230 card..

\textcolor{magenta}{change this figure}
\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.99\textwidth]{./pictures/mrf_cpci_evg230.eps}
  \caption{
...
The cPCI-EVG-230  has a SFP transceiver as an output to EVR/fan-out, one trigger input, one RF input and two universal I/O slots.
...
\begin{table}[!hb]
  ...
  \begin{tabular}{l|l|l}
    ...
    cPCI crate                & \textcolor{magenta}{no info}&               \\\midrule
...
Figure~\ref{fig:diagram} shows the physical hardware setup and Figure~\ref{fig:hw-setup} shows the PCIe-MRF-300 and the IFB-300 setup in the lab.\textcolor{magenta}{Change figures} 
%% \begin{figure}[!t]
%%   \centering
...
\begin{figure}[!b]
  \centering
  \includegraphics[width=0.8\textwidth]{./pictures/evgoncrate.eps}
...
\begin{table}[!htb]
  ...
  \begin{tabular}{l|l}
    ...
    mrf kernel module  & version : \texttt{1} / srcversion \texttt{124F2C1F3D5E2080AE1B755}   \textcolor{magenta}{not sure about this}  \\\midrule
...
Table~\ref{table:fwinfo} shows EVG FPGA Firmware Version Register.\textcolor{magenta}{only EVG FPGA Firmware Version Register is the actual one, change the rest}
...
\begin{table}[!htb]
  ...
  \end{tabular}
  \caption[]{EVR FPGA Firmware Version Register in Reference \citep[see][p66]{MRFEVENTSYSTEMDC}. The EVR in the ICS Lab does not have the Delay Compensation firmware, but the Modular Register Map firmware. \textcolor{magenta}{change this accordingly}}
  \label{table:fwinfo}
\end{table}
...
\begin{lstlisting}[style=termstyle]
  /opt/epics/modules/mrfioc2/3.0.0/db/evg-cpci.db
\end{lstlisting}


\subsection{Start-Up Script}
Listing~\ref{list:st.cmd} shows the IOC start-up script which has the MRF PCIe-EVR-300 Identification Numbers, shown in Table~\ref{table:pciidnumber}. Note that the start-up script is located in the directory in Listing~\ref{list:pwd}.
...
/opt/epics/bases/base-3.14.12.5/bin/centos7-x86_64/softIoc -D /opt/epics/bases/base-3.14.12.5/dbd/softIoc.dbd /tmp/iocsh.startup.3755
#date="Thu 16 Aug 14:24:23 UTC 2016"
...
\textcolor{magenta}{Change this table}
\begin{table}[!htb]
  \centering
...
\section{Step 1 : Check the event frequency}
Short comments on each command or a series of commands are shown before the corresponding command.
\begin{lstlisting}[style=termstyle]

#
# Check the event frequency
#
ics-cpci2-cpu1> dbgf testcPCI-EVG-230-EVG0:EvtClk-Frequency-RB
DBR_DOUBLE:         124.916
\end{lstlisting}

\section{Step 2 : Emmit a software triggered event}
$cPCI-evg-230_1.cmd:$
\begin{lstlisting}[
    style=termstylenumber,
    label={list:evr300-1.cmd},
    caption={Start-up script \texttt{cPCI-evg-230\_1.cmd}. }
  ]
epicsEnvSet("SYS"             "testcPCI-EVG-230")
epicsEnvSet("EVG"             "EVG0")
epicsEnvSet("EVG_PCIBUS"      "0x10")
...
\section{Step 2 : Emmit a sequence}
$ cPCI-evg-230\_2.cmd$
\begin{lstlisting}[ 
    style=termstylenumber,
...
\section{Step 3 : Emmit an event triggered from an output source}
%%  Check if it is this $testcPCI-EVG-230-EVG0:AcTrig-TrigSrc0-SP$ or this $testcPCI-EVG-230-EVG0:UnivInp0-TrigSrc0-SP$.
%% Previoosly need $testcPCI-EVG-230-EVG0:TrigEvt0-EvtCode-SP event_number$
%% \begin{lstlisting}[ 
%%     style=termstylenumber,
